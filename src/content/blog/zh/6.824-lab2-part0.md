...
func (l *Logger) Error(format string, a ...interface{}) {
  if l.level >= LogError {
    prefix := fmt.Sprintf("%08d [ ERROR] ", l.time())
    l.logger.Printf(prefix+format, a...)
  }
}
...
这里我们使用了 `uint8` 来表示日志等级，因而可以直接使用 `>=` 来进行比较。在输出时，我们使用了一个前缀来区分不同的日志等级，这样可以在输出时更加清晰。
...
const (
	...
	dDefault LogTopic = "DEFAULT" // 临时使用的默认主题
)
```

我们使用 `string` 来表示日志主题，因而可以直接和配置的过滤器进行比较，实现为如下的 `Debug` 函数：

```go
func (l *Logger) Debug(topic LogTopic, format string, a ...interface{}) {
  if l.level >= LogDebug {
    if topic != dDefault && !ContainStr(l.filter, string(topic)) {
      prfix := fmt.Sprintf("%08d [%6s] ", l.time(), topic)
      l.logger.Printf(prfix+format, a...)
      return
    } else if topic != dDefault {
      // 被过滤的日志不输出
      return
    }
    prefix := fmt.Sprintf("%08d [ DEBUG] ", l.time())
    l.logger.Printf(prefix+format, a...)
  }
}
...
func (l *Logger) Debug(topic LogTopic, format string, a ...interface{}) {
	if l.level >= LogDebug {
		if topic != dDefault && !ContainStr(l.filter, string(topic)) {
			prefix := ""
			if l.color {
				prefix += fmt.Sprintf("\033[38;5;%dm", ltcMap[topic])
			}
			prefix += fmt.Sprintf("%08d [%6s] ", l.time(), topic)
			l.logger.Printf(prefix+format+"\033[0m\n", a...)
			return
		} else if topic != dDefault {
			return
		}
		prefix := ""
		if l.color {
			prefix += fmt.Sprintf("\033[38;5;%dm", llcMap[LogDebug])
		}
		prefix += fmt.Sprintf("%08d [ DEBUG] ", l.time())
		l.logger.Printf(prefix+format+"\033[0m\n", a...)
	}
...
这里我偷懒没有在 `color` 为 `false` 时删去颜色复位的部分，不过在命令行输出中这并不会有什么影响。
